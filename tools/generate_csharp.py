...
def genWrapFiles():
    wrapParamList = []
    wrapParamList.append(getCmdParams("GameKit", "Runtime.i")) # Rumtime
    ...
    for p in wrapParamList:
        ...
        cmd = "swig -c++ -csharp -namespace %(namespace)s -dllimport %(dll)s -outdir %(outDir)s -o %(wrapFile)s -I%(srcDir)s %(iFile)s" % p
        os.system(cmd)
