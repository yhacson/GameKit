...
def genWrapFiles():
    wrapParamList = []
    wrapParamList.append(getCmdParams("system", "Platform.i")) # system/Platform

    for p in wrapParamList:
        common.mkdirs(p["outDir"])
        common.mkdirs(os.path.dirname(p["wrapFile"]))
        cmd = "swig -c++ -csharp -outdir %(outDir)s -o %(wrapFile)s -I%(srcDir)s %(iFile)s" % p
        os.system(cmd)
